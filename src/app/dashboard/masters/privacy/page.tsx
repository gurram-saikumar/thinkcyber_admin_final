...
export default function PrivacyPage() {
  ...
  // Fetch privacy policies from API
  const fetchPrivacyPolicies = async () => {
    try {
      ...
      // Use the same pattern as terms page
      ...
      if (result.success) {
        const privacyData = Array.isArray(result.data) ? result.data : [];
        ...
        const statsData = {
          ...
          latestVersion: result.stats?.latestVersion || (privacyData.length > 0 
        ...
        console.error('Failed to fetch privacy policies:', result.error);
      }
    } catch (error) {
  ...
  // Create or update privacy policy using apiService
  const handleSavePrivacy = async (privacyData: Omit<PrivacyPolicy, 'id' | 'createdAt' | 'updatedAt' | 'createdBy' | 'updatedBy'>) => {
    try {
      if (selectedPrivacy?.id) {
        ...
        if (result.success) {
          await fetchPrivacyPolicies(); // Refresh the list
  ...
  // Delete privacy policy using apiService
  ...
  // Publish privacy policy using apiService
        ...
        <Card>
          <CardHeader>
            <div className='flex items-center justify-between'>
              <div>
                ...
                <CardDescription>Create, edit, and manage your platform's privacy policies</CardDescription>
              </div>
              <Button
                variant='outline'
          ...
          <CardContent>
            {loading ? (
              ...
              <div className='space-y-4'>
                {privacyPolicies && privacyPolicies.length > 0 && privacyPolicies.map((privacy) => (
